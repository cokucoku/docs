    ...
    <div class="lee-transfer" :class="{filterable:filterable}">
        <div class="lee-transfer-panel">
            <div class="lee-transfer-header">
                <div class="header">
                    <div class="all">
                        <div @click="allleftchoose" class="checkbox" :class="{check:allleft}"><span></span>{{title1}}</div>
                    </div>
                    <div class="js">{{leftxz}}/{{lefttj}}</div>
                </div>
            </div>
            <div class="lee-transfer-body">
                <div class="search" v-if="filterable"><i @click="keyword1=''" v-if="keyword1.length>0"></i><input type="text" name="" v-model="keyword1" placeholder="搜索关键字"></div>
                <ul v-if="leftdata.length>0">
                    <li v-for="item in leftdata">
                        <div @click="item.choose=!item.choose" class="checkbox" :class="{check:item.choose==true}"><span></span>{{item.label}}</div>
                    </li>
                </ul>
                <p v-else>无数据</p>
            </div>
        ...
        <div class="lee-transfer-panel">
            <div class="lee-transfer-header">
                <div class="header">
                    <div class="all">
                        <div @click="allrightchoose" class="checkbox" :class="{check:allright}"><span></span>{{title2}}</div>
                    </div>
                    <div class="js">{{rightxz}}/{{righttj}}</div>
                </div>
            </div>
            <div class="lee-transfer-body">
                <div class="search" v-if="filterable"><i @click="keyword2=''" v-if="keyword2.length>0"></i><input type="text" name="" v-model="keyword2" placeholder="搜索关键字"></div>
...
export default {
    ...
    mounted() {
        ...
        this.list.map(function(el) {
            _this.$set(el, 'choose', false) //choose表示选中状态
            _this.$set(el, 'flag', 0) //flag=0表示在左边，flag=1表示在右边
        })
...
.lee-transfer-panel .lee-transfer-body p {
    text-align: center;margin: 0;padding:12px 0;
}
